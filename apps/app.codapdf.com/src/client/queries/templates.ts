import {
	type TemplateSourceId,
	getTemplateSourceId,
} from "@/app/(main)/templates/_utils/getTemplateSourceId";
import {
	type ReactQueryOptions,
	type RouterInputs,
	trpcClient,
} from "@/server/trpc/trpcClient";

type TemplateSampleByIdQueryOptions =
	ReactQueryOptions["templates"]["getSampleById"];
...
export const useTemplateById = (sourceId: TemplateSourceId) => {
	...
	const templateById = trpcClient.templates.getById.useQuery(
		{ id: templateId ?? -1 },
		{ enabled: !!templateId },
	);
...
export const useTemplateSampleById = (
	input: TemplateSampleByIdInput,
	options?: TemplateSampleByIdQueryOptions,
) => {
	const { data, ...rest } = trpcClient.templates.getSampleById.useQuery(
		input,
		options,
	);
	return {
...
export const useTemplateConvertHtml2PDF =
	trpcClient.templates.convertHtml2PDF.useMutation;
