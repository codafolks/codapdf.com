...
type ComboBoxControllerProps<T extends FieldValues> = Omit<ComboboxBoxProps, "onChange" | "value" | "error"> & {
  id?: string;
  control: Control<T, object>;
  name: Path<T>;
};

const ComboBoxController = <T extends FieldValues>({ name, control, ...rest }: ComboBoxControllerProps<T>) => {
  const {
    fieldState: { error },
